...
//   const ConsultationScreen({super.key, this.patientName, this.appointmentId});
...
//   String _roomId = 'room_page2'; // Different room ID for page 2

//   @override
//   void initState() {
//     super.initState();
//     // For now using a simple room ID - in production, this should be dynamic
//     // based on appointment/patient data
//     _roomId = widget.appointmentId ?? 'room_page2';
//     print('Page 2 - Room ID: $_roomId');
//     _initRenderers();
//     _connectSignaling();
//   }
...
//       print("✅ Offer sent successfully from Page 2");
//     } catch (e, st) {
...
//               ),
//               const SizedBox(height: 24),
//               EndCallButton(onEndCall: _endCall),
//               const SizedBox(height: 16),
...
// class LargerVideoFeed extends StatelessWidget {
...
//           ),
//         ),
//         const Positioned(top: 12, right: 12, child: _VideoTimeAndMute()),
//       ],
//     );
//   }
// }
...
//             subtitle:
//                 '${TimeOfDay.now().format(context)} - ${TimeOfDay.fromDateTime(DateTime.now().add(const Duration(minutes: 30))).format(context)}',
//             isProfile: false,
//           ),
//           const SizedBox(height: 16),
//           _buildDetailRow(
//             icon: Icons.videocam_outlined,
//             text: 'Mode: Video Call',
//             subtitle: 'Appointment ID: $appointmentId',
//             isProfile: false,
//           ),
//         ],
//       ),
...
//         if (isProfile)
//           const CircleAvatar(
//             backgroundImage: AssetImage('assets/dummy_patient_image.png'),
//             radius: 24,
//           )
//         else
//           Container(
...
// class _VideoTimeAndMute extends StatelessWidget {
//   const _VideoTimeAndMute();
...
//   const EndCallButton({super.key, this.onEndCall});
...
// class PrivacyNoticeText extends StatelessWidget {
//   const PrivacyNoticeText({super.key});
...
class _ConsultationScreenState extends State<ConsultationScreen> {
  ...
  void initState() {
    super.initState();
    // Use the SAME fixed room ID for testing
    _roomId = 'test_room_123';
    print('Page 2 (Doctor) - Room ID: $_roomId');
    _initRenderers();
    _connectSignaling();
  }
  ...
  Future<void> _createPeer() async {
    try {
      ...
      _pc!.onTrack = (event) {
        print('🎥 Doctor received remote track: ${event.track.kind}');
        if (event.streams.isNotEmpty) {
          setState(() {
            _remoteRenderer.srcObject = event.streams[0];
          });
          print('✅ Doctor set remote stream to renderer');
        }
      };

      // Handle ICE candidates
      _pc!.onIceCandidate = (RTCIceCandidate candidate) {
        print('🔄 Doctor sending ICE candidate');
        _channel?.sink.add(
          jsonEncode({
            'type': 'candidate',
            'candidate': {
      ...
      _pc!.onConnectionState = (RTCPeerConnectionState state) {
        print('🔗 Doctor connection state: $state');
      };

      _pc!.onIceConnectionState = (RTCIceConnectionState state) {
        print('🧊 Doctor ICE connection state: $state');
      };
    } catch (e, st) {
      print("❌ Error creating peer connection: $e");
      print(st);
  ...
  Future<void> _makeCall() async {
    try {
      ...
      print("✅ Offer sent successfully from Doctor");
    } catch (e, st) {
      print("❌ Error making call: $e");
      print(st);
    }
